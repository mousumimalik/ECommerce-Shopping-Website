...
let serviceAccount = require("./ecom-website-5bde4-firebase-adminsdk-mnuht-5814581f31.json");
...
app.post("/login", (req, res) => {
    ...
    if(!email.length || !password.length) {
        return res.json({"alert": "fill in the inputs"});
    }

    db.collection("users").doc(email).get().then(user => {
        if(!user.exists) { // if email does not exists
            return res.json({"alert": "log in email doesn't exists"});
        }
        else {
